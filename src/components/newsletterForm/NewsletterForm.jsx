...
export const NewsletterForm = () => {
  ...
  const ejsService = process.env.REACT_APP_EMAIL_JS_SERVICE;
  const ejsKey = process.env.REACT_APP_EMAIL_JS_KEY;
  ...
  const sendEmail = (e) => {
    ...
    try {
      emailjs
        .sendForm(ejsService, "template_r9qt37s", emailForm.current, ejsKey)
        .then(
          (result) => {
  ...
  return (
    ...
    >
      <div className="vapetasia-form-fields-wrapper vapetasia-labels-">
        <div className="vapetasia-field-type-email vapetasia-field-group vapetasia-column vapetasia-field-group-email vapetasia-col-100 vapetasia-field-required">
          <input
            ...
            className="vapetasia-field vapetasia-size-sm  vapetasia-field-textual"
            placeholder="Email"
            required="required"
            aria-required="true"
            disabled={submitState !== "initial"}
          />
        </div>
        <div className="vapetasia-field-group vapetasia-column vapetasia-field-type-submit vapetasia-col-100 e-form__buttons">
          <button
            type="submit"
            className="vapetasia-button-newsletter vapetasia-button vapetasia-size-sm"
            disabled={submitState !== "initial"}
          >
            <span>
      ...
      {formError !== "" ? <div>{formError}</div> : <div></div>}
    </form>
  );
};
